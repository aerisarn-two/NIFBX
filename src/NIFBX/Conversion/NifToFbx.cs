...
namespace NIFBX.Conversion
{
    ...
    public sealed class NifToFbx(NifModel model, NifToFbxOptions? options = null)
    {
        ...
        private void ConvertGeometry(FbxScene scene, NifItem shape, FbxObject? parent)
        {
            ...
            // holder carries it (see BakedTransformOf). The clusters are written against
            // this placement -- see FbxSkinIO -- so the two agree about where the mesh
            // stands and nothing places it twice.
            FbxObject holder = FbxMeshWriter.AddModel(
        ...
        /// the second row.
        /// </remarks>
        private NifTransform BakedTransformOf(NifItem shape) => NifTransform.Identity;
